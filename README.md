...
![Tampermonkey](https://img.shields.io/badge/Tampermonkey-compatible-green) ![Version](https://img.shields.io/badge/version-2.9.7-blue)
...
### v2.9.7
- **Team/company filter disappearing bug fix** — selecting "No team" or "No company" no longer causes the filter box to vanish; the dropdown stays visible and can be cleared normally
...
- **Resize handle** — drag the hatched grip in the bottom-right corner of the modal to resize it to any size; size is saved to localStorage and restored on next open
- **New Tab button** — opens the dashboard in a new Intercom tab with the modal auto-opened

### v2.9.4
- **All Open filter** — new filter card showing all open conversations regardless of assignee
...
- **Assignee column** — new optional column showing the admin assigned to each conversation
- **Version footer with update check** — footer now displays current version and an "Update available" link when a newer version is published on GitHub
- **Settings back button** — added ← Back button to close Settings without saving
...
- **Fewer redundant re-renders** — company and urgency filter UIs only rebuild when available options actually change
- **Deduplicated API calls** — admins list is fetched once on modal open instead of twice
- **Arrow indicator inline** — the ▾ chevron now sits inside the admin chip text for consistent placement
